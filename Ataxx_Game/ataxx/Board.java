...
class Board {
    ...
    Board() {
        ...
        for (char c = 'a' - 2; c < 'a' - 2 + EXTENDED_SIDE; c++) {
            for (char r = '1' - 2; r < '1' - 2 + EXTENDED_SIDE; r++) {
                if ((c < 'a' || c >= 'a' + SIDE)
                        || (r < '1' || r >= '1' + SIDE)) {
    ...
    void clear() {
        _whoseMove = RED;
        for (char c = 'a'; c < 'a' + SIDE; c++) {
            for (char r = '1'; r < '1' + SIDE; r++) {
    ...
    boolean legalMove(Move move) {
        return move != null
                && (move.isPass()
                || (get(move.col0(), move.row0()).equals(whoseMove())
                && get(move.col1(), move.row1()).equals(EMPTY)
                && (abs(move.col1() - move.col0()) <= 2
                && abs(move.row1() - move.row0()) <= 2)));
    }
        ...
        for (char c = 'a'; c < 'a' + SIDE; c++) {
            for (char r = '1'; r < '1' + SIDE; r++) {
                if (get(c, r).equals(who)) {
                    for (char c0 = ((char) (c - 2));
                         c0 <= ((char) (c + 2)); c0++) {
                        for (char r0 = ((char) (r - 2));
                             r0 <= ((char) (r + 2)); r0++) {
                            if (get(c0, r0).equals(EMPTY)) {
                                return true;
                            }
                        }
                    }
    ...
    void makeMove(Move move) {
        ...
        for (char c = ((char) (move.col1() - 1));
             c <= ((char) (move.col1() + 1)); c++) {
            for (char r = ((char) (move.row1() - 1));
                 r <= ((char) (move.row1() + 1)); r++) {
                if (get(c, r).equals(opponent)) {
                    set(c, r, whoseMove());
                    incrPieces(whoseMove(), 1);
                    incrPieces(opponent, -1);
                    _undoPieces.push(opponent);
                }
            }
        }
    ...
    void pass() {
        if (canMove(_whoseMove)) {
            throw error("player tried to pass when trying to move");
        }
    ...
    void undo() {
        while (_undoSquares.peek() != null) {
            if (get(_undoSquares.peek()).equals(RED)
                    || get(_undoSquares.peek()).equals(BLUE)) {
                incrPieces(get(_undoSquares.peek()), -1);
            }
            if (_undoPieces.peek().equals(RED)
                    || _undoPieces.peek().equals(BLUE)) {
    ...
    boolean legalBlock(char c, char r) {
        return get(c, r).equals(EMPTY) && numMoves() == 0;
    }
    ...
    void setBlock(char c, char r) {
        ...
        _totalOpen--;
        if (c != (char) ('a' + SIDE / 2)) {
            unrecordedSet((char) ('a' + SIDE + 'a' - c - 1), r, BLOCKED);
            _totalOpen--;
        }
        if (r != (char) ('1' + SIDE / 2)) {
            unrecordedSet(c, (char) ('1' + SIDE + '1' - r - 1), BLOCKED);
            _totalOpen--;
        }
        if (c != (char) ('a' + SIDE / 2) && r != (char) ('1' + SIDE / 2)) {
            unrecordedSet((char) ('a' + SIDE + 'a' - c - 1),
                    (char) ('1' + SIDE + '1' - r - 1), BLOCKED);
            _totalOpen--;
        }
