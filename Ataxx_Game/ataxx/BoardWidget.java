    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        if (_selectedCol != 0 && _selectedRow != 0) {
            ...
            g.fillRect((_selectedCol - 'a') * SQDIM,
                    (SIDE - (_selectedRow - '1') - 1) * SQDIM,
                    SQDIM, SQDIM);
        }
        for (char c = 'a'; c < 'a' + SIDE; c++) {
            for (char r = '1'; r < '1' + SIDE; r++) {
                if (_model.get(c, r).equals(BLOCKED)) {
                    drawBlock(g,
                            (c - 'a') * SQDIM + ((SQDIM - BLOCK_WIDTH) / 2),
                            (SIDE - (r - '1') - 1) * SQDIM
                                    + ((SQDIM - BLOCK_WIDTH) / 2));
                } else if (_model.get(c, r).equals(RED)) {
                    g.setColor(RED_COLOR);
                    g.fillOval((c - 'a') * SQDIM
                                    + ((SQDIM - 2 * PIECE_RADIUS) / 2),
                            (SIDE - (r - '1') - 1) * SQDIM
                                    + ((SQDIM - 2 * PIECE_RADIUS) / 2),
                            PIECE_RADIUS * 2, PIECE_RADIUS * 2);
                } else if (_model.get(c, r).equals(BLUE)) {
